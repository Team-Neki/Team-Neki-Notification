...
- `domain/.../application/port/out/NotificationHistStore.kt` (`interface NotificationHistStore`) — 발송 **성공** 알림을 백엔드 소유 `tb_notification_hist`(앱 "최근 알림" 피드)에 기록하는 포트(H-3). `notification_log`(우리 소유·중복방지 SoT)와 별개 관심사: hist는 유저가 실제 받은 알림 노출용이라 성공분만·best-effort. `type`은 `NotificationType.name`으로 저장(우리 enum명 그대로 — 앱 type 코드 렌더 정합은 앱팀 확인). `link`(딥링크)는 현재 배치 알림에 없어 호출부가 null을 넘김. 외부 소유 테이블이지만 도메인 포트는 프레임워크 무관이라 `NotificationType`만 참조. **주의**: 포트 메서드에 default 파라미터 값을 두지 않는다 — 인터페이스 default는 Kotlin이 비-인터페이스 `$DefaultImpls`를 만들어 `outboundPortsAreInterfaces` ArchUnit 규칙을 깬다.
...
- `apps/batch/.../batch/adapter/out/NotificationHistStoreAdapter.kt` (`class NotificationHistStoreAdapter`, **H-3**) — `NotificationHistStore`의 jOOQ 구현. `tb_notification_hist`는 백엔드(Team-Neki-Server V22) 소유 외부 테이블이라 codegen 대상이 아니라 이름 기반 참조(`DSL.table`/`DSL.field`)로 INSERT(`tb_notification` 리더와 동일 방식). `id`·`created_at`·`updated_at`은 DB DEFAULT에 위임하고 우리는 `user_id`·`type`(=enum명)·`title`·`body`·`link`만 채운다. **best-effort 격리**: 적재를 `@Transactional(REQUIRES_NEW)`로 청크 트랜잭션과 **별도 커넥션**에서 수행한다 — PostgreSQL은 트랜잭션 내 한 statement라도 실패하면 트랜잭션 전체를 abort하므로, 같은 청크 트랜잭션에서 hist INSERT가 실패하면 커밋 시 이미 적재한 `notification_log`까지 롤백된다. 별도 트랜잭션이라야 hist 실패가 커넥션을 오염시키지 않아 호출자(`NotificationSendService`)의 try/catch가 예외를 삼켜 발송·`notification_log`를 지킬 수 있다(검증: `NotificationHistBestEffortTest`).
...
- `apps/batch/.../batch/application/NotificationSendService.kt` (`class NotificationSendService`) — 알림 **1건 처리** 유스케이스(batch-design §5). Spring Batch와 무관하게 "1건을 어떻게 준비/발송하는가"만 담당한다. `prepare(target, type, businessDate)` = 당일 중복 조회(`NotificationLogStore.alreadySent`) + 도메인 판정(`NotificationProcessor.decide`) → `PreparedNotification?`(Skip이면 null). `dispatch(prepared)` = `PushSender.send` → 결과로 `NotificationLogStore.save`(전 결과) → **SUCCESS면 `recordHist`로 `tb_notification_hist` 적재**(H-3). "여러 건을 어떤 단위로 읽고 커밋하는가"(페이징·청크·**트랜잭션 경계**)는 이 서비스가 아니라 배치 어댑터(adapter/in/batch, `CHUNK_SIZE=1`)의 책임이므로, `dispatch`의 send-then-save dual-write 롤백 위험(B-5/M-3)은 호출자 청크가 1건으로 한정한다. 배치 외 트리거(예: 어드민 단건 발송)에서도 재사용 가능.
  - `recordHist(prepared)` (**H-3**) — 발송 성공분을 앱 "최근 알림" 피드용 `tb_notification_hist`에 best-effort 적재한다. hist는 유저가 실제 받은 알림만 보여야 하므로 SUCCESS 한정(FAILED/SKIPPED 제외)이고, `notification_log`(전 결과 적재)와 다르다. 외부(백엔드) 소유 테이블이라 스키마 드리프트 등으로 실패할 수 있는데, 그 실패가 핵심 파이프라인을 깨선 안 되므로 `try/catch`로 예외를 흡수하고 경고만 남긴다(커넥션 격리는 어댑터의 REQUIRES_NEW가 담당 — code-notes 어댑터 항목). `link`는 현재 null. 단위 검증: `NotificationSendServiceTest`.
